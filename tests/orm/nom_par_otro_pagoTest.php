...
class nom_par_otro_pagoTest extends test {
    ...
    public function test_elimina_bd(): void
    {
        ...
        if(errores::$error){
            $error = (new errores())->error(mensaje: 'Error al eliminar $nom_par_percepcion_modelo', data: $del_nom_par_percepcion);
            print_r($error);
            exit;
        }


        $del_nom_par_otro_pago = $nom_nom_par_otro_pago_modelo->elimina_todo();
        if(errores::$error){
        ...
        if(errores::$error){
            $error = (new errores())->error(mensaje: 'Error al eliminar $fc_cfd_partida', data: $del_cfd_partida);
            print_r($error);
            exit;
        }

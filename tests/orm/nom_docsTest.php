...
class nom_docsTest extends test {
    ...
    public function test_alta_bd(){
        ...
        if(errores::$error){
            $error = (new errores())->error('Error al insertar percepcion', $alta);
            print_r($error);
            exit;
        }
        ...
        $nom_docs = array();
        $nom_docs['doc_documento_id'] = 1;
        $nom_docs['nom_nomina_id'] = 1;

        $modelo->registro = $nom_docs;
